...
on the trained Final Coupled XGBoost model.
"""
...
@st.cache_resource
def load_coupled_model():
    m = xgb.XGBRegressor()
    m.load_model(MODEL_PATH)
    return m

def render_what_if_simulator(base_row: pd.Series):
    st.markdown("## 🧪 Weather What-If Simulator")
    st.markdown("Modify meteorological controls to simulate atmospheric changes and observe real-time predictions from the trained **Final Coupled XGBoost model**.")
    
    model = load_coupled_model()
    ...
    with col1:
        ...
        sim_wind_speed = st.slider(
            ...
            value=float(base_row["wind_speed"]),
            step=0.1,
        ...
        sim_humidity = st.slider(
            "Relative Humidity (%)",
            min_value=20, max_value=98,
            value=int(base_row["humidity"]),
            step=1,
        ...
        sim_temperature = st.slider(
            "Temperature (°C)",
            min_value=4.0, max_value=45.0,
            value=float(base_row["temperature"]),
            step=0.5
        )
    ...
    # Recompute explicit coupling interaction terms
    sim_df = compute_coupling_features(sim_df)
    
    # Predict using trained Final Coupled XGBoost model
    ...
    with col2:
        ...
        st.write(f"**Scenario Ventilation Capacity**: `{sim_vc:.0f} m²/s`")
        if sim_vc < 1500:
